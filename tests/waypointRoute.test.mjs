...
// sortCandidates 로직을 재현해 자전거 시간이 제한을 살짝 초과해도 통과하는지 확인
...
function sortCandidates(list, bikeLimitSec = Infinity) {
  const filtered = list.filter(r => calcBikeTime(r.summary) * 60 <= bikeLimitSec + 120);
...
const candidate = {
  summary: {
    subPath: [
      ...
      { trafficType: 4, sectionTime: 15 }, // 자전거 15분
...
const result = sortCandidates([candidate], 900); // 제한 15분(900초)
assert.strictEqual(result.length, 1);
assert.deepStrictEqual(result[0].summary.subPath.map(sp => sp.trafficType), [3, 4, 2, 3]);
